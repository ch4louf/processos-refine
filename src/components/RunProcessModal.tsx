...
import { Play, X, Info, Calendar, AlertTriangle, RefreshCw } from 'lucide-react';
import { calculateStatus, FreshnessStatus, getDaysUntilExpiration } from '../services/governance';
...
interface RunProcessModalProps {
  ...
  onRefresh?: () => void; // NEW: Callback to trigger refresh flow
}

const RunProcessModal: React.FC<RunProcessModalProps> = ({ process, activeRunsCount, onClose, onConfirm, onRefresh }) => {
  ...
  const isExpired = freshnessStatus === 'EXPIRED';
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden scale-100 animate-in zoom-in-95 duration-300 border border-white/20">
        ...
        <div className="px-8 py-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
          <h3 className="font-bold text-slate-900 flex items-center gap-3">
            <div className={`p-2 rounded-xl ${isExpired ? 'bg-red-100 text-red-600' : 'bg-indigo-100 text-indigo-600'}`}>
                {isExpired ? <AlertTriangle size={18} /> : <Play size={18} fill="currentColor" />}
            </div>
            {isExpired ? 'Process Expired' : 'Execute Run'}
          </h3>
        ...
        {isExpired ? (
          <div className="p-8 space-y-6">
            <div className="p-5 rounded-2xl bg-red-50 border-2 border-red-200">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs mt-2 text-red-700/80 leading-relaxed">
                    New runs are blocked because this process expired <strong>{Math.abs(daysUntilExpiration)} days ago</strong>. 
                    The process must be refreshed to ensure all steps, assignees, and governance settings are still accurate.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100">
              <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Process Details</div>
              <div className="text-base font-bold text-slate-900">{process.title}</div>
              <div className="text-xs text-slate-500 mt-1">Category: {process.category}</div>
            </div>
            ...
            <div className="flex justify-end gap-3">
              ...
              {onRefresh && (
                <button 
                  onClick={onRefresh}
                  className="px-6 py-3 text-sm font-black text-white bg-red-600 hover:bg-red-700 rounded-2xl shadow-xl shadow-red-100 transition-all flex items-center gap-2"
                >
                  <RefreshCw size={16} />
                  Refresh Process
                </button>
